...
@section('title', 'Edit Employee ' . $employee->name . ' - E-Learning Cakrawala')
...
@section('container')
    <div class="flex-column container" id="container">
        <div class="flex-row container-row">
            <form action="{{route('employeeEdit', $employee->id)}}" method="POST" class="flex-column container-form" enctype="multipart/form-data">
                @csrf
                <div class="flex-column form-title">
                    <h3 class="poppins">Edit Employee {{Str::limit($employee->name, 20, $end='...')}}</h3>
                    <p class="montserrat">Isilah dengan teliti pada data-data edit akun employee di bawah ini!</p>
                </div>
                @if ($errors->any())
                ...
                <div class="flex-column form-input">
                    <div class="formel" id="input-id">
                        <input type="text" name="username" id="username" placeholder="Username" value="{{old('username') != null ? old('username') : $employee->nip}}">
                    </div>
                    ...
                    <div class="formel" id="input-phone">
                        <input type="text" name="phone" id="phone" placeholder="Nomor HP" value="{{old('phone') != null ? old('phone') : $employee->phone}}">
                    </div>
                    <div class="formel" id="input-status">
                        <select name="status" id="status">
                            <option disabled {{(old('status') == null and $employee->status == null) ? 'selected' : ''}}>Status</option>
                            <option {{old('status') == 'Tenaga Didik' ? 'selected' : ($employee->status == 'Tenaga Didik' ? 'selected' : '')}} value="Tenaga Didik">Tenaga Didik</option>
                            <option {{old('status') == 'Honorer' ? 'selected' : ($employee->status == 'Honorer' ? 'selected' : '')}} value="Honorer">Honorer</option>
                        </select>
                    </div>
                    <div class="formel flex-row">
                        <div class="formel" id="input-salary">
                            <input type="number" name="salary" id="salary" placeholder="Gaji" value="{{old('salary') != null ? old('salary') : $employee->salary}}">
                        </div>
                        <div class="formel" id="input-date">
                            @if (old('tenure') == null and $employee->tenure == null)
                                <input type="text" onfocus="(this.type='month')" name="tenure" id="tenure" placeholder="Tahun masuk">
                            @else
                                <input type="month" name="tenure" id="tenure" placeholder="Tahun masuk" value="{{old('tenure') != null ? old('tenure') : $employee->tenure}}">
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="formel" id="input-address">
                        <textarea name="address" id="address" placeholder="Alamat" cols="30" rows="10">{{old('address') != null ? old('address') : $employee->address}}</textarea>
                    </div>
                </div>
